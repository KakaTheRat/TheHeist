// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "Interactions/InteractableComponent.h"
#include "Player/PlayerInteractionComponent.h"
#include "Kismet/KismetMathLibrary.h"

#include "HideableComponent.generated.h"

/**
 * 
 */
UCLASS(ClassGroup=(Custom), meta=(BlueprintSpawnableComponent))
class THEHEIST_API UHideableComponent : public UInteractableComponent
{
	GENERATED_BODY()

protected:

	//VARIABLES
	
	UPROPERTY(EditAnywhere, Instanced, BlueprintReadWrite, Category="Interactions")
	UHideableData* CurrentInteractionData;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Interactions")
	FVector HiddenLocation;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Interactions")
	FVector QuitLocation;

	AActor* PlayerRef;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Interaction")
	TSubclassOf<UPlayerInteractionComponent> InteractionComponentClass;

	UPlayerInteractionComponent* PlayerComp;

	FTimerHandle HideTimerHandle;
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Hiding")
	float HideTickRate = 0.01f;
	
	//FUNCTIONS

	virtual void BeginPlay() override;

	void Hide();

	void Quit();
	
	//Interaction logic
	virtual void RunInteraction(USceneComponent* HitComponent, UInteractionData* Data) override;

	void HidingInteract();

public :
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Hideable")
	USceneComponent* StartPosition;
};
